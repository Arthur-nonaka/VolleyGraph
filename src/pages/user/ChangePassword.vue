        ...
        <form @submit.prevent="alterarSenha">
          ...
          <div v-if="form.novaSenha" class="password-validation">
            <div
              ...
              :class="{ valid: senhaValida.comprimento }"
            >
              <span class="validation-icon">{{
                senhaValida.comprimento ? "✓" : "✗"
              }}</span>
              Pelo menos 6 caracteres
            </div>
            <div
              v-if="form.confirmarSenha"
              class="validation-item"
              :class="{ valid: senhaValida.confirmacao }"
            >
              <span class="validation-icon">{{
                senhaValida.confirmacao ? "✓" : "✗"
              }}</span>
              As senhas coincidem
            </div>
          </div>
          ...
          <hr />
          <div class="btns">
            <button type="button" class="btn-cancel" @click="limparFormulario">
              Cancelar
            </button>
            <input
              class="form-save"
...
const formularioValido = computed(() => {
  return (
    form.value.senhaAtual.length > 0 &&
